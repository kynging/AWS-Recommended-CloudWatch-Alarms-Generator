...
def lambda_handler(event, context):
    ...
    for m in metrics:
        ...
        if len(dimensions) == 2:
            ...
            db_cluster_identifier = {i['Name']: i['Value'] for i in dimensions}['DBClusterIdentifier']
            ...
            alarms_template['Resources'][metric_name.replace('%', '') + db_cluster_identifier.replace('-', '')] = t
            print(namespace, metric_name, role, db_cluster_identifier)
        elif len(dimensions) == 1:
            if dimensions[0]['Name'] == 'DBInstanceIdentifier':
                db_instance_identifier = {i['Name']: i['Value'] for i in dimensions}['DBInstanceIdentifier']
                t = copy.deepcopy(template['Resources'][metric_name.replace('%', '')])
                t['Properties']['AlarmName'] = t['Properties']['AlarmName'] + db_instance_identifier
                t['Properties']['Dimensions'] = dimensions
                alarms_template['Resources'][metric_name.replace('%', '') + db_instance_identifier.replace('-', '')] = t
