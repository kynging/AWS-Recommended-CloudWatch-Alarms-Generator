...
def lambda_handler(event, context):
    ...
    logger.info("Starting CWAgent alarm generation Lambda function")
        ...
        for r in resources:
            ...
            if metric_name == 'disk_used_percent':
                ...
                if device == 'tmpfs' or device == 'devtmpfs':
                    logger.info(f"Skipping tmpfs/devtmpfs device {device} for instance {instance_id}")
                    alarms_skipped += 1
                    continue
                fstype = [x for x in dimensions if x['Name']=='fstype'][0]['Value']
                if fstype != 'xfs':
                    logger.info(f"Skipping non-xfs filesystem type {fstype} for instance {instance_id}")
                    alarms_skipped += 1
                    continue
