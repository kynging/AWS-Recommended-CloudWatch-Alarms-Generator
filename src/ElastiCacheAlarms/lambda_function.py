...
def lambda_handler(event, context):
    ...
    logger.info("Starting ElastiCache alarm generation Lambda function")
    ...
    for m in metrics:
        ...
        for r in resources:
            ...
            try:
                ...
                logger.warning(f"Cache cluster not found: {cache_cluster_id} in namespace {namespace}")
                alarms_skipped += 1
                continue
            except botocore.exceptions.ClientError as e:
                logger.error(f"Failed to describe cache cluster {cache_cluster_id}: {e.response['Error']['Message']}")
                tags = ''
                alarms_skipped += 1
                continue
            except Exception as e:
                logger.error(f"Unexpected error getting tags for cache cluster {cache_cluster_id}: {e}", exc_info=True)
                tags = ''
                alarms_skipped += 1
                continue
